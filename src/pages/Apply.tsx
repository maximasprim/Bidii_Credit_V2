...
import { COVERED_COUNTIES, OTHER_COUNTY_VALUE } from "../lib/counties";
        ...
        {step === 2 && (
          <form
            onSubmit={handleSubmit((data) => {
              if (data.county === OTHER_COUNTY_VALUE) {
                toast.error(
                  "We're sorry, we don't currently cover loan applications from your area yet. Stay on the lookout for updates ! "
                    // `We're currently serving ${COVERED_COUNTIES.join(", ")} counties.`
                );
                return;
              }
              setDetails(data);
          ...
          >
            ...
            <div>
              <label className="mb-1.5 block text-sm text-ink-500">County</label>
              <select
                {...register("county")}
                defaultValue=""
                className="w-full rounded-xl bg-surface border border-mist-200 px-4 py-2.5 text-sm focus:outline-none"
              >
                ...
                <option value={OTHER_COUNTY_VALUE}>Other (not listed)</option>
              </select>
              {errors.county && <p className="mt-1 text-xs text-red-500">{errors.county.message}</p>}
            </div>
            <div>
              <label className="mb-1.5 block text-xs text-ink-500 lg:text-sm">Where are you based? (town, estate, or area)</label>
              <input
                {...register("location")}
                placeholder="e.g. Ruiru town, or Umoja Estate, Nairobi CBD"
