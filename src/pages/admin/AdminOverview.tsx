...
export default function AdminOverview() {
  ...
  const branchScoped = role === "branch_office_admin" || role === "loan_officer";
  ...
  // The backend already zeroes out contacts/career figures entirely for
  // branch_office_admin and loan_officer (see GET /api/admin/stats) since
  // neither role has menu access to those areas - this just avoids
  // rendering cards/panels of zeros that would look like a bug rather
  // than "not applicable to your role".
  const cards = branchScoped
    ? [{ label: "Loan Applications (your branch)", value: stats.total_loan_applications, recent: stats.loan_applications_last_7_days }]
    : [
    ...
    <div className="space-y-6">
      {branchScoped && (
        <p className="text-xs text-ink-500">
          Showing figures for your branch. 
        </p>
      )}
      ...
      <div className={`grid gap-6 ${branchScoped ? "" : "lg:grid-cols-2"}`}>
        <div className="rounded-2xl border border-mist-200 bg-surface p-5">
          <h2 className="mb-4 font-display text-sm font-bold" style={{ color: "var(--color-ink-900)" }}>
