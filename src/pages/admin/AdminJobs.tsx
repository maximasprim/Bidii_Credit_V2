...
/** The API's ordered list -> a textarea value (one item per line). */
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setCreateForm({ ...createForm, requirementsText: e.target.value })}
                rows={4}
                placeholder={"Bachelor's degree in a relevant field\n2+ years' experience in credit/lending\nValid driving license"}
                className="w-full rounded-xl border border-mist-200 px-4 py-2.5 text-sm focus:outline-none"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setCreateForm({ ...createForm, responsibilitiesText: e.target.value })}
                rows={4}
                placeholder={"Assess and process loan applications\nManage a portfolio of borrowers\nConduct field visits and appraisals"}
                className="w-full rounded-xl border border-mist-200 px-4 py-2.5 text-sm focus:outline-none"
              />
            </div>
          </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        {(job.requirements.length > 0 || job.responsibilities.length > 0) && (
                          <p className="mt-1 text-xs text-ink-500">
                            {job.requirements.length} requirement{job.requirements.length === 1 ? "" : "s"} · {job.responsibilities.length} responsibilit{job.responsibilities.length === 1 ? "y" : "ies"}
                          </p>
                        )}
                        <Link
