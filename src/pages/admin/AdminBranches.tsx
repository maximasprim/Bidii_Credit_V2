...
const emptyForm: BranchForm = { name: "", address: "", hours: "Mon–Fri 8:00–17:00", phone: "", lat: "", lng: "", display_order: "0", county: "" };
...
function BranchFieldsGrid({ form, onChange }: { form: BranchForm; onChange: (f: BranchForm) => void }) {
  ...
  return (
    <div className="grid gap-3 grid-cols-1 lg:grid-cols-3">
      <input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Branch name" className="rounded-lg border border-mist-200 px-3 py-2 text-sm focus:outline-none lg:col-span-2" />
      ...
      <select value={form.county} onChange={(e) => set("county", e.target.value)} className="rounded-lg border border-mist-200 px-3 py-2 text-sm text-ink-700 focus:outline-none">
        <option value="">County (not set - won't be matched by county-based routing)</option>
        {COVERED_COUNTIES.map((c) => <option key={c} value={c}>{c}</option>)}
      </select>
      <input value={form.address} onChange={(e) => set("address", e.target.value)} placeholder="Address" className="rounded-lg border border-mist-200 px-3 py-2 text-sm focus:outline-none" />
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {branch.county ? (
                            <span className="rounded-full bg-mist-100 px-2 py-0.5 text-[10px] font-medium text-ink-600">{branch.county}</span>
                          ) : (
                            <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-medium text-amber-700">No county set</span>
                          )}
                        </div>
