...
import { pathAllowed, resolveAllowedMenus, getLandingPath } from "../../lib/roleAccess";
...
export default function AdminLayout() {
  ...
  // Bounces to this role's own first allowed page, not a hardcoded "/admin"
  // — Overview ("/admin") is itself a configurable menu entry now (see
  // MENU_REGISTRY), so a role with Overview turned off would otherwise
  // infinite-loop: redirected to "/admin", found not allowed, redirected
  // to "/admin" again, forever.
  if (role && !pathAllowed(allowedMenus, location.pathname)) {
    return <Navigate to={getLandingPath(allowedMenus)} replace />;
  }
    ...
    <div className="flex h-screen flex-col overflow-hidden" style={{ backgroundColor: "var(--color-mist-50)" }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="min-w-0 flex-1 overflow-y-auto">
          <div className="mx-auto max-w-8xl px-2 py-6 lg:px-3">
            <Outlet />
          </div>
        </main>
      </div>
...
//         <main className="min-w-0 flex-1 overflow-y-auto">
//           <div className="mx-auto max-w-7xl px-2 py-6 md:px-3 lg:px-3">
//             <Outlet />
//           </div>
//         </main>
//       </div>
